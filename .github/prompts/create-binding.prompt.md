...
**`<SdkName>.Maui.csproj`** (same for both types):
```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
...
Use this path when the SDK provides non-visual functionality (analytics, auth, networking, etc.).
...
Use this path when the SDK provides a visual component (animation view, video player, map, ad banner, etc.). MAUI uses the **Handler pattern** to map cross-platform controls to native views.

**`<SdkName>View.cs`** — Cross-platform view (shared, no platform code):
```csharp
using Microsoft.Maui.Controls;

namespace <SdkName>Maui;

public class <SdkName>View : View
{
    // Bindable properties for the view's configuration
    // Example for a Lottie-like animation view:
    public static readonly BindableProperty SourceProperty =
        BindableProperty.Create(nameof(Source), typeof(string), typeof(<SdkName>View), default(string));
    ...
    // Add BindableProperties for each configurable aspect of the native view
    // (e.g., IsLooping, AutoPlay, Speed, etc.)
    // Add events for callbacks from the native view
    // (e.g., public event EventHandler? AnimationCompleted;)
}
```

Key rules for the cross-platform view:
- Inherit from `Microsoft.Maui.Controls.View`
- Use `BindableProperty` for all configurable properties — this enables XAML data binding
- Use C# `event` for callbacks from the native view
- Keep this file **platform-agnostic** — no native types
- Map the most important native properties (from Phase 3 research)

**`<SdkName>ViewHandler.cs`** — Shared handler mapping (partial class):
```csharp
#if IOS || ANDROID
...
public partial class <SdkName>ViewHandler : ViewHandler<<SdkName>View,
#if IOS
    global::<NativeIOSViewType>   // e.g., LOTAnimationView, MKMapView
#elif ANDROID
    global::<NativeAndroidViewType>  // e.g., Com.Airbnb.Lottie.LottieAnimationView
#endif
>
{
    public static readonly IPropertyMapper<SdkName>View, <SdkName>ViewHandler> Mapper =
        new PropertyMapper<<SdkName>View, <SdkName>ViewHandler>(ViewHandler.ViewMapper)
        {
            [nameof(<SdkName>View.Source)] = MapSource,
            // Map each BindableProperty to a static Map* method
        };

    public <SdkName>ViewHandler() : base(Mapper) { }
}
#endif
```

**`Platforms/iOS/<SdkName>ViewHandler.cs`** — iOS handler implementation:
```csharp
using Microsoft.Maui.Handlers;
using UIKit;
...
public partial class <SdkName>ViewHandler
{
    protected override <NativeIOSViewType> CreatePlatformView()
    {
        // Create and return the native iOS view
        return new <NativeIOSViewType>();
    }

    private static void MapSource(<SdkName>ViewHandler handler, <SdkName>View view)
    {
        // Apply the cross-platform property to the native view
        // handler.PlatformView.<nativeProperty> = view.Source;
    }

    // One static Map* method per BindableProperty
}
```

**`Platforms/Android/<SdkName>ViewHandler.cs`** — Android handler implementation:
```csharp
using Microsoft.Maui.Handlers;
...
public partial class <SdkName>ViewHandler
{
    protected override <NativeAndroidViewType> CreatePlatformView()
    {
        // Android views need Context
        return new <NativeAndroidViewType>(Context);
    }

    private static void MapSource(<SdkName>ViewHandler handler, <SdkName>View view)
    {
        // Apply the cross-platform property to the native view
        // handler.PlatformView.<nativeMethod>(view.Source);
    }
...
**`<SdkName>ViewExtensions.cs`** — Handler registration:
```csharp
namespace <SdkName>Maui;

public static class <SdkName>ViewExtensions
{
    public static MauiAppBuilder Use<SdkName>(this MauiAppBuilder builder)
    {
        builder.ConfigureMauiHandlers(handlers =>
        {
            handlers.AddHandler<<SdkName>View, <SdkName>ViewHandler>();
        });
        return builder;
    }
}
...
Key rules for view handlers:
- Use **partial classes** — the shared handler file defines the mapper, platform files implement `CreatePlatformView` and `Map*` methods
- The generic type parameter of `ViewHandler<TVirtualView, TPlatformView>` must be the actual native view type (from the binding project)
- Use `#if IOS` / `#elif ANDROID` in the shared handler to select the platform view type
- Each `BindableProperty` in the cross-platform view needs a corresponding `Map*` static method in the handler
- For events/callbacks: subscribe in `ConnectHandler`, unsubscribe in `DisconnectHandler`
- Android native views always need `Context` — use `Context` property available in the handler
- Use `global::` prefix for binding namespace types to avoid ambiguity
...
**Additional tests for type = `view`** (when unified project exists):
- **`ViewTests.cs`** — Verify the cross-platform view file:
  - Contains a class inheriting from `View`
  - Has `BindableProperty` declarations for each configurable property
  - Has matching CLR property wrappers using `GetValue`/`SetValue`
- **`HandlerTests.cs`** — Verify the handler files:
  - Shared handler has `IPropertyMapper` with entries for each `BindableProperty`
  - Platform handler files contain `CreatePlatformView` method
  - Each mapped property has a corresponding `Map*` static method
  - Extensions file registers the handler via `AddHandler`
